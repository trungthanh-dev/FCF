...
# ---------------------------------------------------------------------------
# Same delta-target fix as main_tcn_poseidon_delta.py / main_rf_poseidon_delta.py
# (see those files for the full reasoning), applied to XGBoost. Like Random
# Forest, XGBoost is tree-based (no gradient descent through a neural net),
# so none of the Adam/NaN training risk applies here.
#
# Writes to its own results/model/prediction paths so the raw-target
# baseline (main_power.py's run_xgboost_experiment output) stays untouched.
# ---------------------------------------------------------------------------

EDA_DIR = "eda_output_power"
DATA_DIR = "data_clean_power"
PLOT_DIR = os.path.join(EDA_DIR, "xgb_poseidon_delta_plots")
MODEL_DIR = os.path.join("models_saved_power", "xgb_poseidon_delta")
PRED_DIR = os.path.join("predictions_cache_power", "xgb_poseidon_delta")
RESULTS_CSV = os.path.join(EDA_DIR, "xgb_poseidon_delta_results.csv")
...
for horizon in FORECAST_HORIZONS:
    ...
    print(f"\n[XGB-delta] Poseidon | Horizon = {horizon}")
    ...
    plot_actual_vs_predicted(
        ...
        title=f"[XGB-delta] Actual vs Predicted — Poseidon, horizon={horizon}",
        save_path=os.path.join(PLOT_DIR, f"{tag}_actual_vs_pred.png"),
    )
    plot_residuals(
        y_test_raw, y_pred_raw,
        title=f"[XGB-delta] Residuals — Poseidon, horizon={horizon}",
        save_path=os.path.join(PLOT_DIR, f"{tag}_residuals.png"),
    )
    plot_trajectory(
        y_test_raw, y_pred_raw,
        title=f"[XGB-delta] ShaftPower Trajectory: Actual vs. Predicted — Poseidon, horizon={horizon}",
...
print(f"\n[XGB-delta] Full results [MAE/RMSE/DTW in {UNIT_LABEL}]:")
print(results_df)
print("\nCompare against eda_output_power/xgb_results.csv (Poseidon rows).")
